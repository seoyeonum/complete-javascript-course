...
/* 
Julia and Kate are still studying dogs. This time they are want to figure out
if the dogs in their are eating too much or too little food.
...
and for each dog, calculate the recommended food portion (recFood)
and add it to the object as a new property. Do NOT create a new array,
simply loop over the array (We never did this before,
so think about how you can do this without creating a new array).

2. Find Sarah's dog and log to the console whether it's eating too much or too little.
HINT: Some dogs have multiple users, so you first need to find Sarah
...
const ownersTooLittle = dogs
  .filter(dog => dog.curFood < dog.recFood)
  .flatMap(dog => dog.owners);
console.log(ownersTooLittle); // → ['Alice', 'Bob', 'Michael']

// 4. Log a string to the console for each array created in 3., like this:
// "Matilda and Alice and Bob's dogs eat too much!"
// and "Sarah and John and Michael's dogs eat too little!"
console.log(`${ownersTooMuch.join(' and ')}'s dogs are eating too much`);
// → Matilda and Sarah and John and Leo's dogs are eating too much
console.log(`${ownersTooLittle.join(' and ')}'s dogs are eating too little`);
// → Alice and Bob and Michael's dogs are eating too little

// 5. Log to the console whether there is ANY dog eating EXACTLY the amount of food
// that is recommended (just true or false)
console.log(dogs.some(dog => dog.curFood === dog.recFood)); // → true
...
console.log(dogs.every(checkEatingOkay)); // → false
