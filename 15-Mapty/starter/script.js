...
class Workout {
  ...
  _setDescription() {
    // **아래 '// prettier-ignore'은 인접한 바로 아래 줄의 prettier 적용을 무시하도록 한다.
    // prettier-ignore
...
class App {
  ...
  _hideForm() {
    ...
    form.style.display = 'none'; // display: grid 지워 transform 효과 (일시) 없애기
    form.classList.add('hidden');
    setTimeout(() => (form.style.display = 'grid'), 1000); // (1초) 이후 display: grid 살리기
  }
  ...
  _newWorkout(e) {
    ...
    // *isFinite: A finite number is one that's not NaN or ±Infinity
